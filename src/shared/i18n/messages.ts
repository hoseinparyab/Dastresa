...
const EN = {
  ...
  summarySectionDesc:
    'Free summaries go through the Dastresa backend (API key stays on the server). Optionally use your own Luma key.',
  summaryApiKey: 'Custom Luma API key (optional)',
  summaryApiKeyDesc: 'Leave empty to use the free backend. Your override is stored only on this browser.',
...
const FA: Record<MessageKey, string> = {
  ...
  summarySectionDesc:
    'خلاصه رایگان از بک‌اند دسترسا می‌آید (کلید روی سرور می‌ماند). در صورت تمایل می‌توانید کلید لومای خودتان را بگذارید.',
  summaryApiKey: 'کلید API لوما (اختیاری)',
  summaryApiKeyDesc: 'خالی بگذارید تا از بک‌اند رایگان استفاده شود. کلید خودتان فقط روی همین مرورگر ذخیره می‌شود.',
